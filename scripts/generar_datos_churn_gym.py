...
# Multiplicador de P(Baja) = r * CHURN_PROB_MULTIPLIER sobre el riesgo latente.
# A mayor valor, menos clientes quedan censurados (event=0) en el dataset de
# supervivencia. 0.6 (valor original) deja ~75% de censura; 1.6 baja al ~43%.
CHURN_PROB_MULTIPLIER = 1.6
...
try:
    ...
    HAS_FAKER = False
    print(
        "⚠  Faker no instalado. Se usarán nombres genéricos (pip install faker para nombres reales)."
    )
...
def fake_name(gender=None):
    if HAS_FAKER:
        return (
            ...
            if gender == "H"
            else fake.first_name_female() if gender == "M" else fake.first_name()
        )
    ...
    rows.append(
        {
            ...
            # competencia
            "competidor_cercano": random.choice(
                ["Anytime Fitness", "Holmes Place", "VivaGym", "McFIT", "Altafit"]
            ),
...
for i in range(N_CUSTOMERS):
    ...
    antiguedad_dias = int(
        clamp(np.random.exponential(500) * (1 - r * 0.4), 30, 365 * 5)
    )
    fecha_alta = TODAY - timedelta(days=antiguedad_dias)

    estado = (
        "Baja" if random.random() < clamp(r * CHURN_PROB_MULTIPLIER, 0, 1) else "Activo"
    )
    ...
    rows.append(
        {
            ...
            "tiempo_hasta_cierre_dias": int(
                clamp(dias_prev * random.uniform(0.5, 1.0), 1, 60)
            ),
        }
    )
...
for i, row in df_contactos.iterrows():
    ...
    probs = [
        a * (1 - r) + b * r
        for a, b in zip(PLAN_PROBS_BAJO_RIESGO, PLAN_PROBS_ALTO_RIESGO)
    ]
...
for i, row in df_contactos.iterrows():
    ...
    campanas_conv = int(
        campanas_rec * clamp(random.gauss(0.2 - r * 0.15, 0.08), 0, 0.8)
    )
    rows.append(
        {
            "cliente_id": row["id"],
...
TIPOS_INCIDENCIA = [
    "Problema con tarjeta de acceso",
    "Error en factura",
    "Queja instalaciones",
    "Solicitud de congelación",
    "Cambio de plan",
    "Incidencia con entrenador",
    "Problema con reserva de clase",
    "Solicitud de baja",
    "Otros",
]
        ...
        rows.append(
            {
                ...
                "tiempo_medio_resolucion_dias": round(
                    clamp(random.gauss(2 + r * 3, 1), 0.5, 20), 1
                ),
    ...
    rows.append(
        {
            ...
            "material_funcional_disponible": random.choice(
                ["Completo", "Parcial", "Limitado"]
            ),
...
CLASES_POSIBLES = [
    ...
    None,
    None,
    None,  # sin clase = entrenamiento libre
]
HORAS = [
    "07:00",
    "07:30",
...
for i, row in df_contactos.iterrows():
    ...
    rows.append(
        {
            "cliente_id": row["id"],
            "dias_desde_ultimo_acceso_app": int(
                clamp(random.gauss(5 + r * 60, 15), 0, 180)
            ),
            ...
            "engagement_score": int(
                clamp(100 * (1 - r) * random.uniform(0.8, 1.2), 5, 100)
            ),
        }
    )
...
ACCIONES = {
    ...
    "bajo": [
        "Email de fidelización estándar",
...
for i, row in df_contactos.iterrows():
    ...
    if nivel == "alto":
        expl = (
            f"El cliente presenta señales críticas: {motivo.lower()}. "
            f"Tendencia de actividad decreciente. Intervención inmediata recomendada."
        )
    elif nivel == "medio":
        expl = (
            f"El cliente muestra indicios de desenganche: {motivo.lower()}. "
            f"Una acción proactiva puede revertir la situación."
        )
    else:
        expl = (
            f"El cliente está en buen estado. Mantener acciones de fidelización. "
...
for j, cid in enumerate(cohort_ids):
    ...
    expl = (
        "El cliente lleva ~3 meses sin acudir al centro tras 6-12 meses de alta, "
...
df_suscripciones = pd.concat(
    [df_suscripciones, df_suscripciones_cohort], ignore_index=True
)
